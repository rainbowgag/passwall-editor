...
while sleep 5; do
	...
	fingerprint="$(uci -q show passwall_device | grep -E '\.(mac|state|admin_macs)=' | sort | md5sum | cut -d ' ' -f 1)"
	if [ "$(uci -q get passwall.@global[0].enabled)" = "1" ] && \
	   ...
	   find /tmp/etc/passwall/acl -type f -name source_list -size +0c 2>/dev/null | grep -q .; then
		...
		if [ "$last_state" != down ]; then nft flush set inet pwc_portal allowed_macs >/dev/null 2>&1 || true; fi
		last_state=down
		last_fingerprint="$fingerprint"
	fi
done
